...
import (
	...
	"github.com/ays/go-ays/accounts/external"
	"github.com/ays/go-ays/accounts/keystore"
	"github.com/ays/go-ays/accounts/scwallet"
	"github.com/ays/go-ays/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of ays is "ays". If no
...
// These resources are resolved differently for "ays" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by ays 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'ays' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-ays-keystore")
		ephemeral = keydir
	}
