...
type wallet struct {
	...
	deriveChain     ays.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain ays.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
