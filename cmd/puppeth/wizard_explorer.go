...
func (w *wizard) deployExplorer() {
	...
	if w.conf.ethereumtats == "" {
		log.Error("No aystats server configured")
		return
	}
	...
	fmt.Println()
	if infos.node.ethereumtats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.ethereumtats = w.readString() + ":" + w.conf.ethereumtats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.ethereumtats)
		infos.node.ethereumtats = w.readDefaultString(infos.node.ethereumtats) + ":" + w.conf.ethereumtats
	}
