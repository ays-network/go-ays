...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'ays --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec ays --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --aystats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
