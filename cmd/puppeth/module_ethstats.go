...
// aystatsDockerfile is the Dockerfile required to build an aystats backend
// and associated monitoring site.
var aystatsDockerfile = `
FROM puppeth/aystats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// aystatsComposefile is the docker-compose.yml file required to deploy and
// maintain an aystats monitoring site.
var aystatsComposefile = `
version: '2'
services:
  aystats:
    build: .
    image: {{.Network}}/aystats
    container_name: {{.Network}}_aystats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new aystats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(aystatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(aystatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the aystats service
	if nocache {
...
// aystatsInfos is returned from an aystats status check to allow reporting
// various configuration parameters.
type aystatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *aystatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an aystats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*aystatsInfos, error) {
	// Inspect a possible aystats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_aystats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
