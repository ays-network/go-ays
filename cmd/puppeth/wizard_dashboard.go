...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"aystats", "explorer", "wallet", "faucet"} {
	...
	// If we have aystats running, ask whether to make the secret public or not
	if w.conf.ethereumtats != "" {
		fmt.Println()
		fmt.Println("Include aystats secret on dashboard (y/n)? (default = yes)")
