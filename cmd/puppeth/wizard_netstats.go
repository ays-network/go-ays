...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for aystats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["aystats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["aystats"] = infos.Report()
		aystats = infos.config
	}
