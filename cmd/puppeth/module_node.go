...
{{end}}
RUN \
  echo 'ays --cache 512 init /genesis.json' > ays.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ethereum/keystore/ && cp /signer.json /root/.ethereum/keystore/' >> ays.sh && \{{end}}
	echo $'exec ays --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --aystats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> ays.sh

ENTRYPOINT ["/bin/sh", "ays.sh"]
`
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethstats":   config.ethereumtats[:strings.Index(config.ethereumtats, ":")],
		"Etherbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.ethereumtats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ays --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		...
		aystats:   infos.envvars["STATS_NAME"],
