...
import (
	...
	"github.com/ays/go-ays/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	...
	if !ctx.GlobalIsSet(nodeURLFlag.Name) {
		...
		head, err := aysclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
...
func publish(ctx *cli.Context) error {
	...
	head, err := aysclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := aysclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
