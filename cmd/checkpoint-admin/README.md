...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/ays/devp2p/blob/master/caps/les.md).*
...
There are several hardcoded checkpoints in the [source code](https://github.com/ays/go-ays/blob/master/params/config.go#L38) of the go-ays project. These checkpoints are updated by go-ays developers when new versions of software are released. Because light client users trust Geth developers to some extent, hardcoded checkpoints in the code can also be considered correct.
...
```shell
go get github.com/ays/go-ays/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://geth.ays.org/docs/clef/tutorial) .
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/ays/go-ays/blob/master/params/config.go#L251).*
