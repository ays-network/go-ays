...
func newClient(ctx *cli.Context) *aysclient.Client {
	client, err := aysclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, aysclient.NewClient(client))
	if err != nil {
