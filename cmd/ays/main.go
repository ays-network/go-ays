...
// ays is the official command-line client for Ethereum.
package main
...
import (
	...
	"github.com/ays/go-ays/cmd/utils"
	"github.com/ays/go-ays/common"
	"github.com/ays/go-ays/console/prompt"
	"github.com/ays/go-ays/eth"
	"github.com/ays/go-ays/eth/downloader"
	"github.com/ays/go-ays/aysclient"
	"github.com/ays/go-ays/internal/debug"
...
const (
	clientIdentifier = "ays" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-ays command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2020 The go-ays Authors"
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Ays on mainnet...")
	}
...
// ays is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func ays(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local ays node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	ethClient := aysclient.NewClient(rpcClient)
