...
import (
	...
	"github.com/ays/go-ays/consensus/misc"
...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches ays in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches ays in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func retesteth(ctx *cli.Context) error {
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "ays", "debug", "web3"}, srv, false)
	if err != nil {
