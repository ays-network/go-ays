...
func (g *aysrpc) killAndWait() {
	g.ays.Kill()
	g.ays.WaitExit()
}

func (g *aysrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.ays.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *aysrpc) addPeer(peer *aysrpc) {
	g.ays.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.ays.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.ays.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.ays.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.ays.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *aysrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *aysrpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "eth_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.ays.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "ays", ch, "syncing")
	if err != nil {
		g.ays.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.ays.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.ays.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.ays.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.ays.Fatalf("%v timeout syncing", g.name)
		break
	}
}

func startGethWithIpc(t *testing.T, name string, args ...string) *aysrpc {
	g := &aysrpc{name: name}
	args = append([]string{"--networkid=42", "--port=0", "--nousb"}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)
	g.ays = runGeth(t, args...)
	// wait before we can attach to it. TODO: probe for it properly
	time.Sleep(1 * time.Second)
	var err error
	ipcpath := filepath.Join(g.ays.Datadir, "ays.ipc")
...
func startLightServer(t *testing.T) *aysrpc {
	datadir := initGeth(t)
...
func startClient(t *testing.T, name string) *aysrpc {
	datadir := initGeth(t)
