...
func TestAccountListEmpty(t *testing.T) {
	ays := runGeth(t, "account", "list")
	ays.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	ays := runGeth(t, "account", "list", "--datadir", datadir)
	defer ays.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	ays := runGeth(t, "account", "new", "--lightkdf")
	defer ays.ExpectExit()
	ays.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	ays := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer ays.ExpectExit()
	ays.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	ays := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer ays.ExpectExit()
	ays.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(ays.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(ays.StderrText(), m) {
...
// https://github.com/ays/go-ays/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	ays.SetTemplateFunc("keypath", func(file string) string {
