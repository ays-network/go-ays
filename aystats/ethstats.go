...
// Package aystats implements the network stats reporting service.
package aystats
...
// backend encompasses the bare-minimum functionality needed for aystats reporting
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(aystats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["ays"]; info != nil {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the aystats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to aystats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to aystats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to aystats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to aystats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to aystats")
