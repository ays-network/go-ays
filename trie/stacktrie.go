...
func stackTrieFromPool(db aysdb.KeyValueStore) *StackTrie {
...
type StackTrie struct {
	...
	db aysdb.KeyValueStore // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db aysdb.KeyValueStore) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func newLeaf(ko int, key, val []byte, db aysdb.KeyValueStore) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db aysdb.KeyValueStore) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)
