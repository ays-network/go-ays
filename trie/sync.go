...
type Sync struct {
	database aysdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database aysdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them toayser
...
func (s *Sync) Commit(dbw aysdb.Batch) error {
