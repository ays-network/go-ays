// Copyright 2014 The go-ays Authors
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // Whether to disable trie write caching and GC altoayser (archive node)
...
type BlockChain struct {
	...
	db     aysdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db aysdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db aysdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db aysdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain altoayser, unless the freezer is broken
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= aysdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - aysdb.IdealBatchSize)
			}
