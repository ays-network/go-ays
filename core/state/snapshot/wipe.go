...
func wipeSnapshot(db aysdb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db aysdb.KeyValueStore) error {
...
func wipeKeyRange(db aysdb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions toayser to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
