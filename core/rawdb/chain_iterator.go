...
func InitDatabaseFromFreezer(db aysdb.Database) {
...
func iterateTransactions(db aysdb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db aysdb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db aysdb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
