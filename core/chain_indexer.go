...
type ChainIndexer struct {
	chainDb  aysdb.Database      // Chain database to index the data from
	indexDb  aysdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb aysdb.Database, indexDb aysdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
