...
import (
	...
	"github.com/ays/go-ays/common/bitutil"
...
func GetChtRoot(db aysdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db aysdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    aysdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db aysdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
...
func GetBloomTrieRoot(db aysdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db aysdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db aysdb.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *core.ChainIndexer {
