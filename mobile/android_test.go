...
public class AndroidTest extends InstrumentationTestCase {
	...
	// signatures too. Regression test for go-ays issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoayser
	...
	workspace, err := ioutil.TempDir("", "ays-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ays/aystest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/ays/go-ays/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "ays.aar"), "ays.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ays", "aystest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
