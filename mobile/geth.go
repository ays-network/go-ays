...
// entire API provided by go-ays to reduce the maintenance surface and dev
// complexity.
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("ays init: %v", err)
		}
		// If netstats reporting is requested, do it
		if config.EthereumNetStats != "" {
			if err := aystats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.EthereumNetStats); err != nil {
...
func (n *Node) GetEthereumClient() (client *EthereumClient, _ error) {
	...
	return &EthereumClient{aysclient.NewClient(rpc)}, nil
}
