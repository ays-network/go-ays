...
func (b *testBackend) ChainDb() aysdb.Database {
	return b.db
}
...
func TestPendingLogsSubscription(t *testing.T) {
	...
	var (
		...
		testCases = []struct {
			...
			// match all
			{
				ays.FilterQuery{}, flattenLogs(allLogs),
				nil, nil,
			},
			// match none due to no matching addresses
			{
				ays.FilterQuery{Addresses: []common.Address{{}, notUsedAddress}, Topics: [][]common.Hash{nil}},
				nil,
				nil, nil,
			},
			// match logs based on addresses, ignore topics
			{
				ays.FilterQuery{Addresses: []common.Address{firstAddr}},
				append(flattenLogs(allLogs[:2]), allLogs[5][3]),
				nil, nil,
			},
			// match none due to no matching topics (match with address)
			{
				ays.FilterQuery{Addresses: []common.Address{secondAddr}, Topics: [][]common.Hash{{notUsedTopic}}},
				nil, nil, nil,
			},
			// match logs based on addresses and topics
			{
				ays.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}},
				append(flattenLogs(allLogs[3:5]), allLogs[5][0]),
				nil, nil,
			},
			// match logs based on multiple addresses and "or" topics
			{
				ays.FilterQuery{Addresses: []common.Address{secondAddr, thirdAddress}, Topics: [][]common.Hash{{firstTopic, secondTopic}}},
			...
			{
				ays.FilterQuery{Addresses: []common.Address{firstAddr}, FromBlock: big.NewInt(2), ToBlock: big.NewInt(3)},
				append(flattenLogs(allLogs[:2]), allLogs[5][3]),
				nil, nil,
			},
			// multiple pending logs, should match only 2 topics from the logs in block 5
			{
				ays.FilterQuery{Addresses: []common.Address{thirdAddress}, Topics: [][]common.Hash{{firstTopic, fourthTopic}}},
