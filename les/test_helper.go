...
import (
	...
	"github.com/ays/go-ays/accounts/abi/bind/backends"
	"github.com/ays/go-ays/common"
	"github.com/ays/go-ays/common/mclock"
	"github.com/ays/go-ays/consensus/ethash"
	"github.com/ays/go-ays/contracts/checkpointoracle/contract"
	...
	"github.com/ays/go-ays/les/checkpointoracle"
	"github.com/ays/go-ays/les/flowcontrol"
...
func testIndexers(db aysdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db aysdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db aysdb.Database, peers *clientPeerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
