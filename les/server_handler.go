...
import (
	...
	lps "github.com/ays/go-ays/les/lespay/server"
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb aysdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light ays response", "err", err)
			return err
		default:
		}
